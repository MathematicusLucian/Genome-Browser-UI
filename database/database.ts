...
import {
  ...
  IPatientProfile,
  IPatientGenome,
  IPatientProfileAndGenome,
  ISnpPairsResearch,
  IFullReport,
  IPatientGenomeVariant,
} from '../models/database'
import {
  demoGeneVariantMapping,
  demoGenome,
  demoHumanGenes,
  demoMultipleGeneVariants,
  demoPatients,
  homoSapiensChromosomes,
...
patientsIndexedDb.version(1).stores({
  ...
  patientGenome:
    '++patientGenomeId, rsid, genotype, patientId, chromosome, position, datetimestamp',
  patientGenomeVariant:
    '++patientGeneVariantId, rsid, genotype, patientId, chromosome, position, datetimestamp, patientGenomeId',
})
...
async function addDemoDataIfDatabaseTablesEmpty() {
  ...
  try {
    ...
    console.log('chromosomesCount', chromosomesCount)
    if (!chromosomesCount) {
    ...
    console.log('patientProfileCount', patientProfileCount)
